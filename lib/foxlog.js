...
class FoxLog {
  ...
  /**
   * Generate alert (high traffic or low traffic) if needed.
   */
  _generateAlert () {
    let now = new Date();
    ...
    const alert = tools.checkAlert(this.avg, this.alertThreshold, this.alerts[this.alerts.length - 1] && this.alerts[this.alerts.length - 1].type);
